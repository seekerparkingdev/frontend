...
const EventProxSecundario = () => {
  ...
  return (
    <div className="p-6 max-w-[1800px] mx-auto mb-4">
      <h1 className="text-3xl font-semibold mb-8 text-[#30364C] font-urbanist">Próximos eventos</h1>

      {/* Desktop Layout */}
      <div className="hidden md:grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 max-w-7xl mx-auto justify-items-center">
        {(showAll ? events : events.slice(0, 3)).map((event) => (
          <div key={event.id} className="bg-white rounded-3xl w-full shadow-md overflow-hidden">
            ...
            <div className="p-4">
              ...
              <button
                className={`w-full py-2 mt-5 px-4 text-xl rounded-3xl text-center ${event.cancelled ? "bg-[#D2D8D9] text-white cursor-not-allowed" : "bg-[#4B6FC7] text-white"}`}
        ...
        {events.map((event) => (
          ...
          >
            ...
            <div className="p-4">
              ...
              <p className="text-[#30364C] text-sm mb-4">{event.date}</p>
              <button
                className={`w-full py-2 mt-5 px-4 text-md rounded-3xl text-center ${event.cancelled ? "bg-[#D2D8D9] text-white cursor-not-allowed" : "bg-[#4B6FC7] text-white"}`}
                disabled={event.cancelled}
              >
                Comprá tu estacionamiento
              </button>
      ...
      <div className="mt-8 text-center hidden md:block">
        <button
          ...
          className={`bg-[#4B6FC7] ${showAll ? "text-white" : "text-[#D4F7FF]"} py-2 px-12 rounded-3xl text-2xl mt-2`}
        >
